{% macro create_data_generation_function() %}

    {% if execute %}

        {% do log("[+] starting macro create_data_generation_function()", info=True) %}
        ...
        {% do log("[+] function generate_sample_flights created", info=True) %}
